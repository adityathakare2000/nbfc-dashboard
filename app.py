...
if page == "Dashboard":
    ...
    if len(at_risk) > 0:
        at_risk["outstanding_display"] = at_risk["outstanding_amount"].apply(lambda x: f"₹{x/100000:.1f}L")
        ...
        st.markdown('<div class="success-box">✅ No accounts at risk in the selected filters.</div>', unsafe_allow_html=True)
    ...
    with ec1:
        ...
        fig_e = px.pie(df["eligibility"].value_counts().reset_index(), names="eligibility", values="count", hole=0.4,
    ...
    st.download_button("⬇️ Download eligibility report", export_df.to_csv(index=False).encode(), "eligibility_report.csv", "text/csv", use_container_width=True)
    ...
    fig_fc = go.Figure()
    fig_fc.add_trace(go.Scatter(x=months, y=[par30, round(par30*1.05,1), round(par30*1.08,1), round(par30*1.03,1)],
        mode="lines+markers", name="PAR 30+ forecast", line=dict(color="#E24B4A", width=2, dash="dot"), marker=dict(size=8)))
    fig_fc.add_trace(go.Scatter(x=months, y=[npa_rate, round(npa_rate*1.08,1), round(npa_rate*1.12,1), round(npa_rate*1.06,1)],
        mode="lines+markers", name="NPA forecast", line=dict(color="#D85A30", width=2, dash="dot"), marker=dict(size=8)))
    fig_fc.add_hline(y=INDUSTRY_PAR, line_dash="dash", line_color="#378ADD", annotation_text=f"Industry PAR avg {INDUSTRY_PAR}%")
    ...
    st.markdown('<div class="warning-box">⚠️ All 14 columns must be present. Names must match exactly — spelling, case, and underscores.</div>', unsafe_allow_html=True)
    ...
    mistakes = [
        ...
        ("Spaces in column names","loan_id not ' loan_id '"),
    ...
    st.markdown('<div class="success-box">✅ <strong>Ready to upload!</strong> Use the Upload button in the sidebar on any page.</div>', unsafe_allow_html=True)