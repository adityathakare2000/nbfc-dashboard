...
def save_portfolio(user_id, filename, df):
    try:
        ...
        if existing.data:
            ...
            supabase.table("portfolio_data").insert({
                "user_id": user_id,
                "filename": filename,
                "data": data_json,
                "row_count": len(df)
...
def load_portfolio(user_id):
    try:
        ...
        if result.data:
            ...
            df = pd.DataFrame(json.loads(record["data"]))
            return df, record["filename"], record["uploaded_at"]
...
def bucket_color(b): return COLOR_MAP.get(b, "#888")
def bucket_bg(b): return {"Current":"#e8f5e9","SMA-0":"#fff8e1","SMA-1":"#fff3e0","SMA-2":"#fdecea","NPA":"#fdecea"}.get(b,"#f5f5f5")

def page_header(title):
    st.markdown(f'<div style="display:flex;align-items:center;gap:12px;margin-bottom:1.5rem;"><span style="font-size:24px;font-weight:700;">{title}</span><span class="org-badge">🏢 {st.session_state.nbfc_name}</span></div>', unsafe_allow_html=True)
...
if not st.session_state.logged_in:
    ...
    with tab1:
        ...
        with col2:
            ...
            <div style="text-align:center;margin-bottom:2rem;">
                <div style="font-size:36px;font-weight:800;background:linear-gradient(135deg,#1a73e8,#6c47ff);-webkit-background-clip:text;-webkit-text-fill-color:transparent;">🔷 Aadilytics</div>
                <div style="font-size:14px;color:#888;margin-top:6px;">Smart credit risk analytics for lending teams</div>
            </div>
            """, unsafe_allow_html=True)
            email = st.text_input("Email address", placeholder="you@nbfc.com", key="login_email")
                    ...
                    try:
                        ...
                        if not result.data:
                            ...
                            if not bcrypt.checkpw(password.encode(), user["password_hash"].encode()):
                                ...
                                st.warning("Your account is pending approval. Please wait for admin approval before logging in.")
                            elif user["status"] == "rejected":
                                st.error("Your account has been rejected. Please contact support.")
                            elif user["valid_until"] and date.fromisoformat(str(user["valid_until"])) < date.today():
                                st.error(f"Your subscription expired on {user['valid_until']}. Please contact Aadilytics to renew.")
                            else:
                                ...
                                log_activity(user["id"], "login", f"Login from {email}")
                                st.rerun()
                    except Exception as e:
    ...
    with tab2:
        ...
        with col2:
            ...
            st.markdown("""
            <div style="text-align:center;margin-bottom:2rem;">
                <div style="font-size:28px;font-weight:700;color:#1a73e8;">Create your account</div>
                <div style="font-size:13px;color:#888;margin-top:6px;">Your account will be reviewed before activation</div>
            </div>
            """, unsafe_allow_html=True)
            su_nbfc = st.text_input("NBFC / Organisation name", placeholder="e.g. Sunrise NBFC Pvt Ltd", key="su_nbfc")
            su_email = st.text_input("Work email address", placeholder="you@nbfc.com", key="su_email")
            su_password = st.text_input("Create password", type="password", placeholder="Min 8 characters", key="su_password")
            su_confirm = st.text_input("Confirm password", type="password", placeholder="Re-enter password", key="su_confirm")
            st.markdown('<div class="info-box">After signing up, your account will be reviewed by the Aadilytics team. You will be notified once access is granted.</div>', unsafe_allow_html=True)
            if st.button("Create account", use_container_width=True, type="primary", key="signup_btn"):
                if not su_nbfc or not su_email or not su_password or not su_confirm:
                    ...
                    try:
                        ...
                        if existing.data:
                            ...
                            hashed = bcrypt.hashpw(su_password.encode(), bcrypt.gensalt()).decode()
                            ...
                            log_activity(None, "signup", f"New signup: {su_email} from {su_nbfc}")
                            st.success("Account created successfully! The Aadilytics team will review your request and get in touch with you shortly.")
                    except Exception as e:
                        st.error(f"Signup error: {e}")
    st.stop()

with st.sidebar:
    st.markdown('<div style="color:#4f9ef7;font-size:20px;font-weight:700;">🔷 Aadilytics</div>', unsafe_allow_html=True)
    st.markdown('<div style="color:#888;font-size:12px;">Credit risk analytics</div>', unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(f'<span class="org-badge">🏢 {st.session_state.nbfc_name}</span>', unsafe_allow_html=True)
    ...
    if uploaded_file:
        ...
        if save_portfolio(st.session_state.user_id, uploaded_file.name, new_df):
            st.success(f"{len(new_df)} records saved to your account")
        else:
            st.warning("Uploaded but could not save to database")
    elif st.session_state.df is None:
        saved_df, saved_filename, saved_at = load_portfolio(st.session_state.user_id)
        if saved_df is not None:
            st.session_state.df = saved_df
            uploaded_at_fmt = saved_at[:10] if saved_at else "unknown"
            st.info(f"Loaded: {saved_filename}\nUploaded: {uploaded_at_fmt}")
        else:
            if st.session_state.role == "admin":
                try:
                ...
                st.session_state.df = pd.DataFrame()
                st.warning("No data yet — please upload your portfolio CSV above")
...
if not has_data and page != "🔧 Admin Panel" and page != "📋 Upload Guide":
    st.markdown(f"# {page}")
    st.markdown('<div class="warning-box"><strong>No portfolio data loaded.</strong> Please upload your CSV file using the sidebar to view analytics.</div>', unsafe_allow_html=True)
    st.markdown("Go to **📋 Upload Guide** for instructions on the required format.")
    st.stop()
...
if page == "📊 Dashboard":
    ...
    if len(sdf) == 0:
        st.markdown('<div class="warning-box">No results found.</div>', unsafe_allow_html=True)
        ...
        for cid in unique_customers:
            ...
            phone_display = str(c_loans["phone"].iloc[0]) if has_phone else "N/A"
            ...
            <div style="background:{wbg};border-radius:12px;padding:20px;margin-bottom:12px;border-left:5px solid {wclr};">
                <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:8px;">
                    <div>
                        ...
                        <div style="font-size:13px;color:#666;margin-top:4px;">{num_loans} loan account{'s' if num_loans > 1 else ''} with {st.session_state.nbfc_name}</div>
                    </div>
            ...
            for idx, (_, loan) in enumerate(c_loans.iterrows()):
                ...
                with st.expander(f"Account {idx+1} — {loan['loan_id']} · {loan['product']} · {bucket} · DPD {dpd}", expanded=(idx==0)):
                    ...
                    with p1:
                        ...
                        fig_emi = go.Figure()
                        fig_emi.add_trace(go.Bar(name="Paid", x=["EMI count","Amount (L)"], y=[emis_paid, round(emi_amt*emis_paid/100000,1)], marker_color="#1D9E75", text=[f"{emis_paid}",f"Rs.{emi_amt*emis_paid/100000:.1f}L"], textposition="outside"))
                        fig_emi.add_trace(go.Bar(name="Remaining", x=["EMI count","Amount (L)"], y=[emis_rem, round(emi_amt*emis_rem/100000,1)], marker_color="#B5D4F4", text=[f"{emis_rem}",f"Rs.{emi_amt*emis_rem/100000:.1f}L"], textposition="outside"))
                        fig_emi.update_layout(barmode="group", height=260, margin=dict(t=20,b=10), plot_bgcolor="white", paper_bgcolor="white", legend=dict(orientation="h",y=1.1))
                        st.plotly_chart(fig_emi, use_container_width=True)
                    with p2:
                        st.markdown("**Repayment breakdown**")
                        fig_rep = go.Figure()
                        fig_rep.add_trace(go.Bar(x=["Repaid","Outstanding"], y=[round(repaid/100000,1), round(outstanding/100000,1)], marker_color=["#1D9E75","#E24B4A"], text=[f"Rs.{repaid/100000:.1f}L ({repaid_pct}%)",f"Rs.{outstanding/100000:.1f}L"], textposition="outside"))
                        fig_rep.update_layout(height=260, margin=dict(t=20,b=10), yaxis_title="Lakhs", plot_bgcolor="white", paper_bgcolor="white", showlegend=False)
                        st.plotly_chart(fig_rep, use_container_width=True)

                    d1, d2 = st.columns(2)
                    with d1:
                        st.markdown(f"""<div style="background:#f8f9fa;border-radius:10px;padding:16px;">
                    ...
                    if bucket == "NPA":
                        ...
                        st.markdown('<div class="danger-box"><strong>High priority collection.</strong> 61-90 DPD. No new loans.</div>', unsafe_allow_html=True)
                    elif bucket == "SMA-1":
                        ...
                        st.markdown('<div class="warning-box"><strong>Early warning.</strong> 1-30 DPD. Contact proactively.</div>', unsafe_allow_html=True)
                    else:
        ...
        for cid in unique_customers:
            ...
            summary.append({"Customer ID": cid if has_cid else "N/A","Borrower": b["borrower_name"].iloc[0],"PAN": b["pan"].iloc[0] if has_pan else "N/A","Accounts": len(b),"Outstanding": f"Rs.{b['outstanding_amount'].sum()/100000:.1f}L","Max DPD": int(b["dpd"].max()),"Worst bucket": worst})
        ...
        st.download_button("Download list", sdf.to_csv(index=False).encode(), "search_results.csv", "text/csv", use_container_width=True)

elif page == "✅ Pre-Disbursal Check":
    page_header("✅ Pre-Disbursal Credit Check")
    st.markdown("Complete credit assessment before disbursing a new loan.")
    st.markdown("---")
    search_input = st.text_input("Search borrower by name, customer ID or PAN", placeholder="e.g. Mehta Traders / CID-10001")
    ...
    if not search_input:
        ...
        if len(results) == 0:
            st.markdown('<div class="warning-box">Borrower not found in system.</div>', unsafe_allow_html=True)
            st.markdown('<div class="success-box"><strong>New customer.</strong> No existing obligations. Proceed with standard credit appraisal.</div>', unsafe_allow_html=True)
        else:
            unique_cids = results[group_col].unique()
            selected_cid = unique_cids[0]
            if len(unique_cids) > 1:
                selected_cid = st.selectbox("Multiple matches — select customer", unique_cids)
            ...
            if npa_accounts > 0:
                verdict, verdict_color, verdict_bg = "REJECT", "#c62828", "#fdecea"
                verdict_reason = f"{npa_accounts} NPA account(s) detected. Cannot extend credit."
            elif worst_bucket == "SMA-2":
                verdict, verdict_color, verdict_bg = "REJECT", "#c62828", "#fdecea"
                verdict_reason = "SMA-2 account detected. High default risk."
            elif worst_bucket == "SMA-1":
                verdict, verdict_color, verdict_bg = "HOLD", "#e65100", "#fff3e0"
                verdict_reason = "SMA-1 account detected. Regularise dues before new disbursement."
            elif worst_bucket == "SMA-0":
                verdict, verdict_color, verdict_bg = "CONDITIONAL", "#f9a825", "#fff8e1"
                verdict_reason = "Minor overdue detected. Proceed with additional collateral only."
            elif avg_bureau >= 700 and repaid_pct >= 40:
                verdict, verdict_color, verdict_bg = "APPROVE", "#2e7d32", "#e8f5e9"
                verdict_reason = "Good repayment history and bureau score. Eligible for new loan."
            elif avg_bureau >= 650:
                verdict, verdict_color, verdict_bg = "APPROVE WITH CAUTION", "#1565c0", "#e8f0fe"
                verdict_reason = "Satisfactory profile. Standard terms with normal monitoring."
            else:
                verdict, verdict_color, verdict_bg = "REVIEW", "#6a1b9a", "#f3e5f5"
                verdict_reason = "Low bureau score. Manual review required."
            ...
            checks_data = [
                ...
                ("Overdue accounts", f"{overdue_accounts}/{total_loans}", "Pass" if overdue_accounts == 0 else "Warning" if overdue_accounts/total_loans < 0.3 else "Fail", "Overdue ratio"),
                ...
                ("Performing accounts", f"{performing_accounts}/{total_loans}", "Pass" if performing_accounts/total_loans >= 0.7 else "Warning", "Min 70% performing"),
                ("Monthly EMI load", f"Rs.{total_emi_obligation:,.0f}", "Pass" if total_emi_obligation < 100000 else "Warning", "Existing load"),
                ("Total exposure", f"Rs.{total_outstanding/100000:.1f}L", "Pass" if total_outstanding < 5000000 else "Warning", "Concentration risk"),
            ]
            ...
            disp["loan_fmt"] = disp["loan_amount"].apply(lambda x: f"Rs.{x/100000:.1f}L")
            ...
            with cc1:
                ...
                fig_exp = go.Figure()
                fig_exp.add_trace(go.Bar(x=["Disbursed","Repaid","Outstanding"], y=[total_disbursed/100000,total_repaid/100000,total_outstanding/100000], marker_color=["#378ADD","#1D9E75","#E24B4A"], text=[f"Rs.{total_disbursed/100000:.1f}L",f"Rs.{total_repaid/100000:.1f}L",f"Rs.{total_outstanding/100000:.1f}L"], textposition="outside"))
                fig_exp.update_layout(height=280, margin=dict(t=20,b=10), yaxis_title="Lakhs", plot_bgcolor="white", paper_bgcolor="white", showlegend=False)
            ...
            if max_dpd > 90: flags.append(("🔴", f"Max DPD {max_dpd} days — high default risk", "danger"))
            if max_dpd > 30: flags.append(("🟠", f"Current max DPD is {max_dpd} days", "warning"))
            if avg_bureau < 650: flags.append(("🔴", f"Bureau score {avg_bureau} below minimum 650", "danger"))
            if avg_bureau < 700: flags.append(("🟡", f"Bureau score {avg_bureau} below preferred 700", "warning"))
            if total_emi_obligation > 100000: flags.append(("🟠", f"Monthly EMI load Rs.{total_emi_obligation:,.0f} is high", "warning"))
            if total_outstanding > 5000000: flags.append(("🟡", f"Total exposure Rs.{total_outstanding/100000:.1f}L — monitor concentration", "warning"))
            if len(flags) == 0:
                st.markdown('<div class="success-box"><strong>No risk flags.</strong> Clean profile — proceed with standard appraisal.</div>', unsafe_allow_html=True)
            else:
    ...
    def categorise(row):
        if row["bucket"] == "NPA" or row["dpd"] > 90:
            return "Blacklisted","🔴","No new loans.","#fdecea","#c62828"
            ...
            return "Risky","🟠","New loans only with collateral.","#fff8e1","#e65100"
        elif row["bucket"] == "SMA-0" or row["dpd"] > 0:
            return "Borderline","🟡","Top-up only. Monitor closely.","#fffde7","#f9a825"
            ...
            return "Eligible","🟢","Eligible at standard terms.","#e8f5e9","#2e7d32"
        else:
    ...
    for _, row in filtered_elig.head(50).iterrows():
        cid_span = f"<span style='font-size:11px;color:#888;margin-left:8px;font-family:monospace;'>{row['customer_id']}</span>" if has_cid else ""
        st.markdown(f"""
        <div style="background:{row['bg']};border-radius:8px;padding:12px 16px;margin-bottom:6px;display:flex;justify-content:space-between;align-items:center;">
            <div><span style="font-weight:600;">{row['icon']} {row['borrower_name']}</span><span style="font-size:12px;color:#666;margin-left:12px;">{row['product']} · {row['geography']}</span>{cid_span}</div>
            <div style="text-align:right;"><span style="font-size:12px;font-weight:600;color:{row['color']};background:white;padding:2px 10px;border-radius:20px;">{row['eligibility']}</span><div style="font-size:11px;color:#888;margin-top:3px;">{row['reason']}</div></div>
        </div>
        """, unsafe_allow_html=True)
    ...
    st.download_button("Download eligibility report", filtered_elig[export_cols].to_csv(index=False).encode(), "eligibility_report.csv", "text/csv", use_container_width=True)

elif page == "🤖 AI Insights":
    page_header("🤖 AI Insights")
    st.caption(f"Analysis based on {len(df):,} loan records from {st.session_state.nbfc_name}")

    par30 = safe_par(df)
    npa_rate = safe_npa(df)
    collection_eff = safe_collection(df)
    avg_bureau = round(df["bureau_score_at_origination"].mean(), 0)
    INDUSTRY_PAR, INDUSTRY_NPA, INDUSTRY_COLLECTION, INDUSTRY_BUREAU = 8.2, 3.1, 88.5, 695

    st.subheader("Portfolio vs industry benchmark")
    m1,m2,m3,m4 = st.columns(4)
    m1.metric("PAR 30+", f"{par30}%", f"{round(par30-INDUSTRY_PAR,1)}% vs industry", delta_color="inverse")
    m2.metric("NPA Rate", f"{npa_rate}%", f"{round(npa_rate-INDUSTRY_NPA,1)}% vs industry", delta_color="inverse")
    m3.metric("Collection Eff.", f"{collection_eff}%", f"{round(collection_eff-INDUSTRY_COLLECTION,1)}% vs industry")
    m4.metric("Avg Bureau Score", f"{int(avg_bureau)}", f"{int(avg_bureau-INDUSTRY_BUREAU)} vs industry")
    ...
    with ai2:
        ...
        fig_h = px.histogram(df, x="bureau_score_at_origination", nbins=20, color_discrete_sequence=["#7F77DD"])
    ...
    if par30 > INDUSTRY_PAR:
        observations.append(("🔴 PAR above industry", f"Your PAR 30+ is {par30}% vs industry {INDUSTRY_PAR}%. Focus collection on SMA-1 and SMA-2.", "danger"))
    else:
        observations.append(("🟢 PAR below industry", f"Your PAR 30+ of {par30}% beats industry {INDUSTRY_PAR}%.", "success"))
    if collection_eff < INDUSTRY_COLLECTION:
        observations.append(("🔴 Collection below average", f"Your {collection_eff}% is below industry {INDUSTRY_COLLECTION}%.", "danger"))
    else:
        observations.append(("🟢 Strong collection", f"Your {collection_eff}% beats industry {INDUSTRY_COLLECTION}%.", "success"))
    ...
    if len(hrg) > 0:
        observations.append(("📍 Highest risk geography", f"{hrg.idxmax()} has highest delinquency.", "warning"))
    lb = df[df["bureau_score_at_origination"] < 650]
    if len(lb) > 0:
        lb_npa = round(len(lb[lb["bucket"]=="NPA"])/len(lb)*100, 1)
        observations.append(("📊 Bureau insight", f"Borrowers below 650 score have {lb_npa}% NPA rate.", "guide"))
    ...
    st.markdown("---")
    st.subheader("NPA prediction by product")
    pred_rows = []
    ...
    fig_fc = go.Figure()
    fig_fc.add_trace(go.Scatter(x=months, y=par_fc, mode="lines+markers+text", name="PAR 30+", line=dict(color="#E24B4A",width=3), marker=dict(size=10), text=[f"{v}%" for v in par_fc], textposition="top center", textfont=dict(size=12,color="#E24B4A")))
    fig_fc.add_trace(go.Scatter(x=months, y=npa_fc, mode="lines+markers+text", name="NPA Rate", line=dict(color="#1D9E75",width=3,dash="dash"), marker=dict(size=10,symbol="diamond"), text=[f"{v}%" for v in npa_fc], textposition="bottom center", textfont=dict(size=12,color="#1D9E75")))
    ...
    admin_tab1, admin_tab2, admin_tab3 = st.tabs(["User Management","Activity Logs","System Info"])
    ...
    with admin_tab1:
        ...
        try:
            ...
            if all_users.data:
                users_df = pd.DataFrame(all_users.data)
                users_df = users_df[["id","email","nbfc_name","role","status","valid_until","created_at"]]

                st.markdown("---")
                pending = users_df[users_df["status"] == "pending"]
                if len(pending) > 0:
                    st.markdown(f'<div class="warning-box"><strong>{len(pending)} pending approval(s)</strong> — review below.</div>', unsafe_allow_html=True)

                for _, user in users_df.iterrows():
                    status_color = {"pending":"#f9a825","approved":"#2e7d32","rejected":"#c62828"}.get(user["status"],"#888")
                    with st.expander(f"{user['nbfc_name']} — {user['email']} — {user['status'].upper()}", expanded=(user["status"]=="pending")):
                        u1,u2,u3,u4 = st.columns(4)
                        u1.metric("NBFC", user["nbfc_name"])
                        u2.metric("Role", user["role"])
                        u3.metric("Status", user["status"])
                        u4.metric("Valid until", str(user["valid_until"]) if user["valid_until"] else "Not set")

                        if user["status"] == "pending":
                            st.markdown("---")
                            ac1, ac2, ac3 = st.columns(3)
                            with ac1:
                                valid_date = st.date_input(f"Set validity for {user['email']}", value=date(2025, 12, 31), key=f"date_{user['id']}")
                            with ac2:
                                role_select = st.selectbox("Role", ["user","admin"], key=f"role_{user['id']}")
                            with ac3:
                                st.markdown("<br>", unsafe_allow_html=True)
                                if st.button(f"Approve", key=f"approve_{user['id']}", type="primary", use_container_width=True):
                                    ...
                                    log_activity(st.session_state.user_id, "approve_user", f"Approved {user['email']} until {valid_date}")
                                    st.success(f"Approved {user['email']} until {valid_date}")
                                    st.rerun()
                            if st.button(f"Reject", key=f"reject_{user['id']}", use_container_width=True):
                                ...
                                log_activity(st.session_state.user_id, "reject_user", f"Rejected {user['email']}")
                            ...
                            ec1, ec2 = st.columns(2)
                            with ec1:
                                new_valid = st.date_input(f"Extend validity", value=date.fromisoformat(str(user["valid_until"])) if user["valid_until"] else date.today(), key=f"extend_{user['id']}")
                            with ec2:
                                st.markdown("<br>", unsafe_allow_html=True)
                                if st.button("Update validity", key=f"update_{user['id']}", use_container_width=True):
                                    supabase.table("users").update({"valid_until": new_valid.isoformat()}).eq("id", user["id"]).execute()
                                    log_activity(st.session_state.user_id, "extend_validity", f"Extended {user['email']} to {new_valid}")
                                    st.success(f"Updated validity to {new_valid}")
                                    st.rerun()
                            if st.button("Revoke access", key=f"revoke_{user['id']}"):
                                supabase.table("users").update({"status": "rejected"}).eq("id", user["id"]).execute()
                                st.error(f"Access revoked for {user['email']}")
                                st.rerun()
        except Exception as e:
    ...
    with admin_tab2:
        ...
        try:
            logs = supabase.table("activity_log").select("*, users(email, nbfc_name)").order("created_at", desc=True).limit(50).execute()
            if logs.data:
                log_rows = []
                for log in logs.data:
                    ...
                    log_rows.append({
                        ...
                        "NBFC": user_info.get("nbfc_name","System"),
    ...
    with admin_tab3:
        ...
        try:
            ...
            total_uploads = supabase.table("portfolio_data").select("id", count="exact").execute()
            si1,si2,si3,si4 = st.columns(4)
            si1.metric("Total users", total_users.count)
            si2.metric("Approved", approved_users.count)
            si3.metric("Pending approval", pending_users.count)
            si4.metric("Total data uploads", total_uploads.count)
        except Exception as e:
            st.error(f"Error: {e}")
    ...
    st.markdown('<div class="warning-box">All 17 columns must be present with exact column names.</div>', unsafe_allow_html=True)
    st.dataframe(pd.DataFrame({
        ...
        "Format":["Unique e.g. LN-1001","Same for all loans of one customer e.g. CID-10001","Full name","PAN card","10-digit mobile","Business Loan/MSME Loan/Equipment Finance/Working Capital/Personal Loan","City","YYYY-MM-DD","Numbers only","Numbers only","Months","% e.g. 18.5","Count","0 if current","Current/SMA-0/SMA-1/SMA-2/NPA","Collected/Partially Collected/Defaulted","300-900"]
    }), use_container_width=True, hide_index=True, height=580)
    st.markdown('<div class="success-box">Upload using the sidebar button on any page. Your data is saved securely and loads automatically on your next login.</div>', unsafe_allow_html=True)