...
USERS = {"demo@nbfc.com": {"password": "demo123",
                           "org": "Sunrise NBFC",
                           "role": "user"},
         "admin@nbfc.com": {"password": "admin123",
                            "org": "Horizon Finance",
                            "role": "admin"},
         "user@finance.com": {"password": "finance1",
                              "org": "Bharat Lending Co",
                              "role": "user"},
         }
...
if not st.session_state.logged_in:
    ...
    with col2:
        ...
        st.markdown(
            '<div class="brand">🔷 Aadilytics</div>',
            unsafe_allow_html=True)
        st.markdown(
            '<div class="brand-sub">Smart risk analytics for lending teams</div>',
            unsafe_allow_html=True)
...
with st.sidebar:
    st.markdown(
        ...
        unsafe_allow_html=True)
    st.markdown(
        '<div class="brand-sub">Smart risk analytics</div>',
        unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(
        f'<span class="org-badge">🏢 {st.session_state.org}</span>', unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("---")
...
def page_header(title):
    st.markdown(f'<div style="display:flex;align-items:center;gap:12px;margin-bottom:1.5rem;"><span style="font-size:24px;font-weight:500;">{
                title}</span><span class="org-badge">🏢 {st.session_state.org}</span></div>', unsafe_allow_html=True)
...
def safe_collection(data):
    if len(data) == 0:
        return 0.0
    return round(len(data[data["collection_status"] ==
                 "Collected"]) / len(data) * 100, 1)
...
if page == "Dashboard":
    ...
    with cf3:
        selected_bucket = st.selectbox(
            "Bucket", ["All"] + ["Current", "SMA-0", "SMA-1", "SMA-2", "NPA"])
    ...
    with ch2:
        ...
        if len(filtered) > 0:
            ...
            for prod in filtered["product"].unique():
                ...
                par_prod.append({"Product": prod,
                                 "PAR 30+ %": safe_par(prod_df),
                                 "Loans": len(prod_df)})
            par_prod_df = pd.DataFrame(par_prod).sort_values(
                "PAR 30+ %", ascending=True)
            fig2 = px.bar(
                par_prod_df,
                x="PAR 30+ %",
                y="Product",
    ...
    with ch3:
        ...
        if len(filtered) > 0:
            geo_aum = filtered.groupby("geography")[
    ...
    with ch4:
        ...
        if len(filtered) > 0:
            bureau_bucket = filtered.groupby(
                "bucket")["bureau_score_at_origination"].mean().round(0).reset_index()
            bureau_bucket.columns = ["Bucket", "Avg Bureau Score"]
            bucket_order_list = ["Current", "SMA-0", "SMA-1", "SMA-2", "NPA"]
            bureau_bucket["Bucket"] = pd.Categorical(
                bureau_bucket["Bucket"], categories=bucket_order_list, ordered=True)
    ...
    st.caption(
        "Account movement between buckets — green = improving, red = worsening")
    bucket_order = ["Current", "SMA-0", "SMA-1", "SMA-2", "NPA"]
    roll_data = {"Current": [88, 9, 2, 1, 0], "SMA-0": [34, 41, 18, 6, 1], "SMA-1": [
        18, 22, 35, 19, 6], "SMA-2": [8, 11, 22, 38, 21], "NPA": [3, 5, 8, 14, 70]}
    ...
    st.dataframe(roll_df.style.map(color_cells).format(
        "{:.0f}%"), use_container_width=True)

    st.markdown("---")
    st.subheader("Early warning — accounts at risk")
    at_risk = filtered[filtered["bucket"].isin(
        ["SMA-2", "NPA"])].copy().sort_values("dpd", ascending=False)
    if len(at_risk) > 0:
        at_risk["outstanding_display"] = at_risk["outstanding_amount"].apply(
            lambda x: f"Rs.{x / 100000:.1f}L")
        at_risk["Risk Level"] = at_risk["bucket"].apply(
            lambda b: "Urgent" if b == "NPA" else "High Risk")
        st.dataframe(at_risk[["loan_id", "borrower_name", "product", "geography", "outstanding_display", "dpd", "bucket", "Risk Level"]].rename(
            columns={"outstanding_display": "outstanding"}).reset_index(drop=True), use_container_width=True, height=280)
    else:
        st.markdown(
            '<div class="success-box">No accounts at risk in selected filters.</div>',
            unsafe_allow_html=True)
    ...
    st.markdown("Search any borrower to see all their accounts with your NBFC.")

    st.markdown("---")
    sc1, sc2, sc3, sc4 = st.columns([2, 2, 1, 1])
    with sc1:
        search_query = st.text_input(
            "Search by name, loan ID or PAN",
            placeholder="e.g. Mehta Traders / LN-5001 / ABCDE1234F")
    with sc2:
    ...
    with sc3:
        filter_bucket = st.selectbox(
            "Filter by bucket", ["All"] + ["Current", "SMA-0", "SMA-1", "SMA-2", "NPA"])
    with sc4:
        filter_geo = st.selectbox(
            "Filter by geography",
            ["All"] +
            sorted(
                df["geography"].unique().tolist()))
    ...
    if search_query:
        mask = (
            search_df["borrower_name"].str.contains(
                search_query,
                case=False,
                na=False) | search_df["loan_id"].str.contains(
                search_query,
                case=False,
                na=False))
        if "pan" in search_df.columns:
            mask = mask | search_df["pan"].str.contains(
                search_query, case=False, na=False)
        if "phone" in search_df.columns:
            mask = mask | search_df["phone"].str.contains(
                search_query, case=False, na=False)
    ...
    if filter_geo != "All":
        search_df = search_df[search_df["geography"] == filter_geo]

    st.markdown(f"**{len(search_df):,} loan accounts found**")
    st.markdown("---")

    if len(search_df) == 0:
        st.markdown(
            '<div class="warning-box">No borrowers found. Try a different name, loan ID, PAN, or phone number.</div>',
            unsafe_allow_html=True)

    else:
        unique_borrowers = search_df["borrower_name"].unique()

        if len(unique_borrowers) == 1 or (
                search_query and len(unique_borrowers) <= 3):
            for borrower_name in unique_borrowers:
                borrower_loans = search_df[search_df["borrower_name"] == borrower_name].copy(
                )
                num_accounts = len(borrower_loans)
                total_outstanding = borrower_loans["outstanding_amount"].sum()
                total_loan_amt = borrower_loans["loan_amount"].sum()
                total_repaid = total_loan_amt - total_outstanding
                avg_dpd = round(borrower_loans["dpd"].mean(), 1)
                max_dpd = int(borrower_loans["dpd"].max())
                avg_bureau = int(
                    borrower_loans["bureau_score_at_origination"].mean())
                worst_bucket = borrower_loans.sort_values(
                    "dpd", ascending=False).iloc[0]["bucket"]
                active_loans = len(
                    borrower_loans[borrower_loans["bucket"] != "NPA"])
                npa_loans = len(
                    borrower_loans[borrower_loans["bucket"] == "NPA"])
                ...
                bucket_bgs = {
                    ...
                    "SMA-2": "#fdecea",
                    "NPA": "#fdecea"}
                risk_clr = bucket_colors.get(worst_bucket, "#888")
                risk_bg = bucket_bgs.get(worst_bucket, "#f5f5f5")
                ...
                <div style="background:{risk_bg};border-radius:12px;padding:20px;margin-bottom:16px;border-left:5px solid {risk_clr};">
                    <div style="display:flex;justify-content:space-between;align-items:flex-start;">
                        <div>
                            <div style="font-size:20px;font-weight:600;color:#1a1a2e;">{borrower_name}</div>
                            <div style="font-size:13px;color:#666;margin-top:4px;">{num_accounts} loan account{'s' if num_accounts > 1 else ''} with {st.session_state.org}</div>
                        </div>
                        <div style="background:{risk_clr};color:white;padding:4px 14px;border-radius:20px;font-size:13px;font-weight:500;">Worst bucket: {worst_bucket}</div>
                    </div>
                </div>
                """, unsafe_allow_html=True)

                km1, km2, km3, km4, km5, km6, km7 = st.columns(7)
                km1.metric("Total accounts", f"{num_accounts}")
                km2.metric("Active accounts", f"{active_loans}")
                km3.metric("NPA accounts", f"{npa_loans}")
                km4.metric(
                    "Total outstanding", f"Rs.{
                        total_outstanding / 100000:.1f}L")
                km5.metric("Total repaid", f"Rs.{total_repaid / 100000:.1f}L")
                km6.metric("Max DPD", f"{max_dpd} days")
                km7.metric("Avg bureau score", f"{avg_bureau}")

                st.markdown("---")
                st.subheader(f"All loan accounts — {borrower_name}")

                for idx, (_, loan) in enumerate(borrower_loans.iterrows()):
                    dpd = int(loan["dpd"])
                    bucket = loan["bucket"]
                    ...
                    emis_remaining = max(tenure - emis_paid, 0)
                    emi_amount = round((loan_amt *
                                        (loan["interest_rate"] /
                                         100 /
                                         12) *
                                        (1 +
                                       ...
                                       ((1 +
                                         loan["interest_rate"] /
                                           100 /
                                           12)**tenure -
                                        1), 0) if tenure > 0 else 0
                    total_paid_amt = emi_amount * emis_paid
                    total_pending_amt = emi_amount * emis_remaining
                    lclr = bucket_colors.get(bucket, "#888")
                    lbg = bucket_bgs.get(bucket, "#f5f5f5")

                    with st.expander(f"Account {idx + 1} — {loan['loan_id']} · {loan['product']} · {bucket}", expanded=(idx == 0)):
                        st.markdown(
                            f"""
                        <div style="background:{lbg};border-radius:8px;padding:12px 16px;margin-bottom:12px;border-left:4px solid {lclr};">
                            <span style="font-size:13px;color:{lclr};font-weight:500;">{bucket}</span>
                            <span style="font-size:13px;color:#666;margin-left:12px;">{
                                loan['product']} &nbsp;·&nbsp; {
                                loan['geography']} &nbsp;·&nbsp; Disbursed {
                                loan['disbursement_date']}</span>
                        </div>
                        """, unsafe_allow_html=True)
                        ...
                        a1.metric(
                            "Loan amount", f"Rs.{
                                loan_amt / 100000:.1f}L")
                        a2.metric(
                            "Outstanding", f"Rs.{
                                outstanding / 100000:.1f}L")
                        a3.metric(
                            "Repaid", f"Rs.{
                                repaid / 100000:.1f}L ({repaid_pct}%)")
                        a4.metric("EMI amount", f"Rs.{emi_amount:,.0f}")
                        a5.metric("Days past due", f"{dpd} days")
                        a6.metric("Interest rate", f"{loan['interest_rate']}%")
                        ...
                        with p1:
                            st.markdown("**EMI & payment progress**")
                            emi_data = pd.DataFrame({
                                "Category": ["EMIs paid", "EMIs remaining"],
                                "Count": [emis_paid, emis_remaining],
                                "Amount (Rs.)": [total_paid_amt, total_pending_amt]
                            })

                            fig_emi = go.Figure()
                            fig_emi.add_trace(go.Bar(
                                name="EMIs paid",
                                x=["EMI count", "Amount (Rs.)"],
                                y=[emis_paid, total_paid_amt / 100000],
                                marker_color="#1D9E75",
                                text=[f"{emis_paid} EMIs", f"Rs.{total_paid_amt / 100000:.1f}L"],
                                textposition="outside"
                            ))
                            fig_emi.add_trace(go.Bar(
                                name="EMIs remaining",
                                x=["EMI count", "Amount (Rs.)"],
                                y=[emis_remaining, total_pending_amt / 100000],
                                marker_color="#B5D4F4",
                                text=[f"{emis_remaining} EMIs", f"Rs.{total_pending_amt / 100000:.1f}L"],
                                textposition="outside"
                            ))
                        ...
                        with p2:
                            ...
                            st.plotly_chart(
                                fig_repay, use_container_width=True)
                        ...
                        with d1:
                            ...
                            <div style="background:#f8f9fa;border-radius:10px;padding:16px;">
                                <table style="width:100%;font-size:13px;">
                                    <tr><td style="color:#666;padding:6px 0;">Loan ID</td><td style="font-weight:500;text-align:right;">{loan['loan_id']}</td></tr>
                                    <tr><td style="color:#666;padding:6px 0;">Product</td><td style="font-weight:500;text-align:right;">{loan['product']}</td></tr>
                                    <tr><td style="color:#666;padding:6px 0;">Disbursement date</td><td style="font-weight:500;text-align:right;">{loan['disbursement_date']}</td></tr>
                                    ...
                                    <tr><td style="color:#666;padding:6px 0;">Interest rate</td><td style="font-weight:500;text-align:right;">{loan['interest_rate']}%</td></tr>
                                    <tr><td style="color:#666;padding:6px 0;">Tenure</td><td style="font-weight:500;text-align:right;">{tenure} months</td></tr>
                                    <tr><td style="color:#666;padding:6px 0;">EMI amount</td><td style="font-weight:500;text-align:right;">Rs.{emi_amount:,.0f}/month</td></tr>
                                </table>
                            </div>
                            """, unsafe_allow_html=True)
                        with d2:
                            ...
                            <div style="background:#f8f9fa;border-radius:10px;padding:16px;">
                                <table style="width:100%;font-size:13px;">
                                    <tr><td style="color:#666;padding:6px 0;">Current bucket</td><td style="font-weight:500;text-align:right;color:{lclr};">{bucket}</td></tr>
                                    <tr><td style="color:#666;padding:6px 0;">Days past due</td><td style="font-weight:500;text-align:right;">{dpd} days</td></tr>
                                    <tr><td style="color:#666;padding:6px 0;">Collection status</td><td style="font-weight:500;text-align:right;">{loan['collection_status']}</td></tr>
                                    <tr><td style="color:#666;padding:6px 0;">Bureau score</td><td style="font-weight:500;text-align:right;">{int(loan['bureau_score_at_origination'])}</td></tr>
                                    <tr><td style="color:#666;padding:6px 0;">EMIs paid</td><td style="font-weight:500;text-align:right;">{emis_paid} of {tenure}</td></tr>
                                    <tr><td style="color:#666;padding:6px 0;">EMIs remaining</td><td style="font-weight:500;text-align:right;">{emis_remaining}</td></tr>
                                    <tr><td style="color:#666;padding:6px 0;">Geography</td><td style="font-weight:500;text-align:right;">{loan['geography']}</td></tr>
                                </table>
                            </div>
                            """, unsafe_allow_html=True)
                        ...
                        if bucket == "NPA":
                            st.markdown(
                                '<div class="danger-box"><strong>Immediate action required.</strong> This account has crossed 90 DPD and is classified as NPA. Initiate legal recovery. Do not extend new credit on this account.</div>',
                                unsafe_allow_html=True)
                        elif bucket == "SMA-2":
                            st.markdown(
                                '<div class="danger-box"><strong>High priority collection.</strong> Account is at 61-90 DPD. Assign senior collection agent immediately. New loans not recommended until this account is regularised.</div>',
                                unsafe_allow_html=True)
                        elif bucket == "SMA-1":
                            st.markdown(
                                '<div class="warning-box"><strong>Active monitoring needed.</strong> Account is at 31-60 DPD. Schedule follow-up call within 48 hours. Restructuring may be considered if borrower shows intent.</div>',
                                unsafe_allow_html=True)
                        elif bucket == "SMA-0":
                            st.markdown(
                                '<div class="warning-box"><strong>Early warning signal.</strong> Account shows first signs of stress at 1-30 DPD. Contact borrower proactively — may self-cure with prompt follow-up.</div>',
                                unsafe_allow_html=True)
                        else:
                            if int(loan["bureau_score_at_origination"]) >= 720:
                                st.markdown(
                                    '<div class="success-box"><strong>Premium account.</strong> Performing well with strong bureau score. Eligible for top-up or higher credit limit.</div>',
                                    unsafe_allow_html=True)
                            else:
                                st.markdown(
                                    '<div class="success-box"><strong>Performing account.</strong> No overdue. Continue standard monitoring cycle.</div>',
                                    unsafe_allow_html=True)

                st.markdown("---")
                st.subheader("Overall borrower summary")
                bs1, bs2 = st.columns(2)
                with bs1:
                    st.markdown("**Accounts by bucket**")
                    bkt_summary = borrower_loans.groupby("bucket").agg(
                        Accounts=("loan_id", "count"),
                        Outstanding=("outstanding_amount", "sum")
                    ).reset_index()
                    bkt_summary["Outstanding"] = bkt_summary["Outstanding"].apply(lambda x: f"Rs.{
                                                                                  x / 100000:.1f}L")
                    st.dataframe(
                        bkt_summary,
                ...
                with bs2:
                    ...
                    prod_summary = borrower_loans.groupby("product").agg(
                        Accounts=("loan_id", "count"),
                        Outstanding=("outstanding_amount", "sum")
                    ).reset_index()
                    prod_summary["Outstanding"] = prod_summary["Outstanding"].apply(
                        lambda x: f"Rs.{x / 100000:.1f}L")
                    st.dataframe(
                        prod_summary,
                        use_container_width=True,
                ...
                export_borrower = borrower_loans[["loan_id",
                                                  "borrower_name",
                                                  "product",
                                                  "geography",
                                                  "loan_amount",
                                                  ...
                                                  "bucket",
                                                  "collection_status",
                                                  "bureau_score_at_origination"]].copy()
                st.download_button(
                    f"Download all accounts for {borrower_name}",
                    export_borrower.to_csv(
                        index=False).encode(),
            ...
            st.subheader("Matching borrowers")
            st.caption("Narrow your search to see individual borrower profiles")

            borrower_summary = []
            for bname in unique_borrowers:
                b_loans = search_df[search_df["borrower_name"] == bname]
                worst = b_loans.sort_values(
                    "dpd", ascending=False).iloc[0]["bucket"]
                borrower_summary.append({
                    "Borrower name": bname,
                    "Total accounts": len(b_loans),
                    "Total outstanding": f"Rs.{b_loans['outstanding_amount'].sum() / 100000:.1f}L",
                    "Max DPD": int(b_loans["dpd"].max()),
                    "Worst bucket": worst,
                    "Products": ", ".join(b_loans["product"].unique()),
                    "Geographies": ", ".join(b_loans["geography"].unique())
                })

            summary_df = pd.DataFrame(borrower_summary)

            def color_bucket_col(val):
                colors = {
                    "Current": "background-color:#e8f5e9",
                    "SMA-0": "background-color:#fff8e1",
                    "SMA-1": "background-color:#fff3e0",
                    "SMA-2": "background-color:#fdecea",
                    "NPA": "background-color:#fdecea"}
            ...
            s1.metric("Unique borrowers", f"{len(unique_borrowers):,}")
            s2.metric("Total loan accounts", f"{len(search_df):,}")
            s3.metric("Total AUM", f"Rs.{safe_aum(search_df)} Cr")
            s4.metric("PAR 30+", f"{safe_par(search_df)}%")
    ...
    st.markdown(
        "Every borrower categorised based on repayment behaviour, DPD, and bureau score.")
    ...
    for cat, col, clr in zip(cats, cols, colors):
        ...
        col.markdown(f'<div style="background:#f8f9fa;border-radius:10px;padding:12px;text-align:center;border-top:3px solid {clr};"><div style="font-size:20px;font-weight:600;color:{
                     clr};">{count}</div><div style="font-size:11px;color:#888;margin-top:2px;">{cat}</div><div style="font-size:11px;color:#aaa;">{pct}%</div></div>', unsafe_allow_html=True)
    ...
    with ec2:
        ...
        df["eligible_flag"] = df["eligibility"].apply(
            lambda x: "Eligible" if x in [
                "Premium", "Eligible"] else "Not eligible")
        prod_e = df.groupby(["product", "eligible_flag"]
    ...
    m1.metric("PAR 30+",
              f"{par30}%",
              f"{round(par30 - INDUSTRY_PAR,
                 1)}% vs industry {INDUSTRY_PAR}%",
              delta_color="inverse")
    m2.metric("NPA Rate",
              f"{npa_rate}%",
              f"{round(npa_rate - INDUSTRY_NPA,
                 1)}% vs industry {INDUSTRY_NPA}%",
              delta_color="inverse")
    m3.metric("Collection Efficiency", f"{collection_eff}%", f"{round(
        collection_eff - INDUSTRY_COLLECTION, 1)}% vs industry {INDUSTRY_COLLECTION}%")
    m4.metric("Avg Bureau Score", f"{int(avg_bureau)}", f"{
              int(avg_bureau - INDUSTRY_BUREAU)} vs industry {INDUSTRY_BUREAU}")
    ...
    with ai1:
        ...
        fig_c = go.Figure()
        fig_c.add_trace(go.Bar(name="Your Portfolio",
                               x=["PAR 30+",
                                  "NPA Rate",
                                  "Collection Eff."],
                               y=[par30,
                                  ...
                                  collection_eff],
                               marker_color="#1D9E75"))
        fig_c.add_trace(go.Bar(name="Industry Avg",
                               x=["PAR 30+",
                                  "NPA Rate",
                                  "Collection Eff."],
                               y=[INDUSTRY_PAR,
    ...
    if par30 > INDUSTRY_PAR:
        observations.append(("🔴 PAR above industry average", f"Your PAR 30+ is {par30}% vs industry {
                            INDUSTRY_PAR}%. Focus collection on SMA-1 and SMA-2 accounts before they slip to NPA.", "danger"))
    else:
        observations.append(
            ("🟢 PAR below industry average",
             f"Your PAR 30+ of {par30}% beats industry average of {INDUSTRY_PAR}%. Credit selection is performing well.",
             "success"))
    if collection_eff < INDUSTRY_COLLECTION:
        observations.append(("🔴 Collection efficiency below average", f"Your {collection_eff}% is below industry {
                            INDUSTRY_COLLECTION}%. Review collection team performance by geography.", "danger"))
    else:
        observations.append(("🟢 Strong collection efficiency", f"Your {collection_eff}% beats industry {
                            INDUSTRY_COLLECTION}%. Operations team is performing well.", "success"))
    high_risk_product = df[df["dpd"] >= 30].groupby("product").size()
    if len(high_risk_product) > 0:
        observations.append(("⚠️ Highest risk product", f"{high_risk_product.idxmax(
        )} has the highest overdue concentration. Consider tightening credit criteria.", "warning"))
    high_risk_geo = df[df["dpd"] >= 30].groupby("geography").size()
    if len(high_risk_geo) > 0:
        observations.append(("📍 Highest risk geography", f"{high_risk_geo.idxmax(
        )} has the highest delinquency rate. Review collection operations in this region.", "warning"))
    low_bureau = df[df["bureau_score_at_origination"] < 650]
    if len(low_bureau) > 0:
        lb_npa = round(
            len(low_bureau[low_bureau["bucket"] == "NPA"]) / len(low_bureau) * 100, 1)
        observations.append(("📊 Bureau score insight", f"Borrowers with score below 650 have {
                            lb_npa}% NPA rate. Consider setting 650 as minimum threshold.", "guide"))
    for title, text, type_ in observations:
        box = {
            "danger": "danger-box",
            "success": "success-box",
            "warning": "warning-box",
            "guide": "guide-box"}[type_]
        st.markdown(
            f'<div class="{box}"><strong>{title}</strong><br>{text}</div>',
            unsafe_allow_html=True)
    ...
    st.markdown(
        '<div class="guide-box">About predictions — Forecasts are directional indicators based on current roll rates. Actual outcomes depend on collection efforts and macro conditions.</div>',
        unsafe_allow_html=True)
    ...
    admin_tab1, admin_tab2, admin_tab3 = st.tabs(
        ["Logic Transparency", "User Management", "System Info"])

    with admin_tab1:
        st.subheader("Calculation logic — all formulas used in Aadilytics")
        st.markdown(
            "This page documents every formula and transformation used across the platform.")
        st.markdown("---")
    ...
    with admin_tab2:
        ...
        st.markdown(
            '<div class="warning-box">This is a demo user list. Full database-backed user management comes in the next version.</div>',
            unsafe_allow_html=True)
        user_data = []
        ...
        if st.button("Add user", use_container_width=True):
            st.markdown(
                '<div class="success-box">User added to demo list. In production this will write to a database.</div>',
                unsafe_allow_html=True)
    ...
    with admin_tab3:
        ...
        for col in df.columns:
            ...
            quality_rows.append({"Column": col,
                                 "Type": str(df[col].dtype),
                                 "Null count": nulls,
                                 "Null %": f"{round(nulls / len(df) * 100,
                                                    1)}%",
    ...
    st.markdown(
        '<div class="guide-box">Download our sample CSV and use it as a template.</div>',
        unsafe_allow_html=True)
    sample_data = {
    ...
    st.markdown(
        '<div class="warning-box">All 14 columns must be present. Names must match exactly.</div>',
        unsafe_allow_html=True)
    st.dataframe(
        pd.DataFrame(
            {
                ...
                "Format": [
                    ...
                    "City e.g. Mumbai",
                    "YYYY-MM-DD",
                    "Numbers only e.g. 500000",
                    "Numbers only e.g. 420000",
                    "Months e.g. 36",
                    "% e.g. 18.5",
                    "Count e.g. 8",
                    "0 if current else 30/60/90/120",
    ...
    for i, (title, desc) in enumerate(mistakes):
        with c1 if i % 2 == 0 else c2:
            st.markdown(
                f'<div class="warning-box"><strong>{title}</strong><br>{desc}</div>',
                unsafe_allow_html=True)
    ...
    st.dataframe(
        ...
        hide_index=True)
    st.markdown(
        '<div class="success-box">Ready to upload! Use the Upload button in the sidebar.</div>',
        unsafe_allow_html=True)
